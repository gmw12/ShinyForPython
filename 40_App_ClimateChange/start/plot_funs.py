...
def plot_country(temp, country = 'Germany', year = 1960):
    ...
    temp_filt_groupsum_all = temp.groupby(['Country', 'Year']).Value.agg('median').reset_index()
    
    # group sum for specified country
    temp_filt_groupsum = temp_filt.groupby('Year').Value.agg('median').reset_index()
    ...
    g = (ggplot(data=temp_filt_groupsum_all) + aes(x='Year', y='Value') 
    + geom_point(color='grey', alpha=0.1)
    + annotate('text', x=1985, y=2, label=f"All countries ({year}): {np.round(temp_filt_all_year, 1)} °C")
    + annotate('text', x=1985, y=2.5, label=f"Selected country ({year}): {np.round(temp_filt_sel_year, 1)} °C", color='red') 
    + scale_color_discrete()
    + geom_smooth() + geom_smooth(data=temp_filt_groupsum, color='red')
    + geom_vline(xintercept=year, linetype='dashed')
    + coord_cartesian(ylim = [-1, 4]) + labs(x = 'Year [-]', y='Temperature Change [deg C]', title=f'Temperature Change for {country}')
    + theme_bw() + coord_cartesian(ylim=[-1, 3])
    + theme(plot_background=element_rect(fill='gray', alpha=0), panel_background=element_rect(fill='gray', alpha=0), 
