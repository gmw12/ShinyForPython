...
from plot_funs import plot_world, plot_country
...
app_ui = ui.page_fluid(
    ...
    ui.row(
        ui.column(6, ui.input_select(id='country', label = "Choose a Country",
            choices = countries)),
        ui.column(6,
            ui.row(
                ui.column(6,
                    ui.input_slider(id='year', label = 'Choose a year',
                    min=temp_year_min, max=temp_year_max, value=temp_year_min,
                    step=1, animate=False) 
                ),
                ui.column(6,
                    ui.output_ui('color_map'),
                ),
            ),
        ),
    ),
...
def server(input, output, session):
    ...
    #when year changes, update the maps zoom attribute
    @reactive.Effect
    def _():
    ...
    async def graph_country():
        with ui.Progress(min=1, max=15) as p:
            p.set(message="Calcing now", detail="Hold it!")
            for i in range(1, 15):
                p.set(i, message="calcing")
                await sleep(0.1)
